  ...
  <li class="flex justify-between items-center gap-5 border-b border-gray-300 pb-6 last:border-0">
    <div class="flex items-center gap-5">
      ...
      <div class="flex flex-col gap-1">
        <h3 class="text-2xl font-bold leading-none" :class="todo.completed ? 'line-through' : ''">{{ todo.title }}</h3>
        <p class="leading-none" :class="todo.completed ? 'line-through' : ''">{{ todo.description }}</p>
      </div>
    </div>
    <Button
        variant=""
